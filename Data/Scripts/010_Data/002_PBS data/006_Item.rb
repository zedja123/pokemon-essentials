module GameData
  class Item
    ...
    def self.icon_filename(item)
      # If the item is nil, return the default back icon
      return "Graphics/Items/back" if item.nil?
    
      # If item is an array, iterate through each item
      if item.is_a?(Array)
        # Collect the icon filenames for all items in the array
        item.map do |individual_item|
          individual_item_data = self.try_get(individual_item)
          next "Graphics/Items/000" if individual_item_data.nil?
    
          # Check for the file corresponding to the item
          ret = sprintf("Graphics/Items/%s", individual_item_data.id)
          return ret if pbResolveBitmap(ret)
    
          # Check for TM/HM type icons if applicable
          if individual_item_data.is_machine?
            prefix = "machine"
            if individual_item_data.is_HM?
              prefix = "machine_hm"
            elsif individual_item_data.is_TR?
              prefix = "machine_tr"
            end
            move_type = GameData::Move.get(individual_item_data.move).type
            type_data = GameData::Type.get(move_type)
            ret = sprintf("Graphics/Items/%s_%s", prefix, type_data.id)
            return ret if pbResolveBitmap(ret)
    
            if !individual_item_data.is_TM?
              ret = sprintf("Graphics/Items/machine_%s", type_data.id)
              return ret if pbResolveBitmap(ret)
            end
          end
    
          # Default to "000" if no file is found
          "Graphics/Items/000"
        end
      else
        # Handle the case where the item is a single item (not an array)
